...
//const videoLoad = document.querySelector(".video")
const html = document.querySelector("html")

gsap.set(".header-spacer", { yPercent: 100 })
gsap.set(".circle-link", { opacity: 0 })
gsap.to(".logo-container-preloader", { opacity: 1, duration: 0.8, delay: 0.5 })

preloader.to(".preloader", { yPercent: -100, duration: 1, delay: 1.8, ease: Expo.easeOut })
   // .to("iframe", { yPercent: 43.5, duration: 1 }, "<")
...
paperHoverDesktop.add("(min-width: 850px)", () => {
    ...
    articles.forEach((article, index) => {
        article.addEventListener("mouseenter", () => {
            gsap.to(readContainer[index], { yPercent: 0, duration: 0.6, ease: "power2.inOut" })
        }, { signal: controller.signal })

        article.addEventListener("mouseleave", () => {
            gsap.to(readContainer[index], { yPercent: -150, duration: 0.6, ease: "power2.inOut" })
        }, { signal: controller.signal })
    })
